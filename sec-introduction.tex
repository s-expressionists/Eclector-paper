...
\item[Cross compilation] When a \cl{} system (the \emph{target}) is
  ...
  different way.
\item[Parsing an editor buffer] As a concrete example of this use
  case, consider the (bad) \cl{} program in
  ...
  notify them about the syntactic errors\footnote{Examples of
    syntactic errors are three package markers in
    \mintinline[escapeinside=||]{lisp}{cl:::nth}, a digit that is
    invalid for the chosen radix in
    \mintinline[escapeinside=||]{lisp}{|\#|b0102} and a missing
  ...
  errors\footnote{Only one branch of the nested read-time conditional
    \mintinline[escapeinside=||]{lisp}{|\#|+sbcl |\#|b0102 |\#|-sbcl
